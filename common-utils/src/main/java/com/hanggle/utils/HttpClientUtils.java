...
public class HttpClientUtils {
    ...
    static {
        ...
        config = ConnectionConfig.custom().setCharset(Charset.forName("UTF-8")).build();
    ...
    public static String get(String url, String param, String charset) {
        ...
        try {
            ...
            logger.error("请求通信[" + url + "]时连接超时,堆栈轨迹如下", cte);
        } catch (SocketTimeoutException ste) {
            logger.error("请求通信[" + url + "]时读取超时,堆栈轨迹如下", ste);
        } catch (ClientProtocolException cpe) {
            // 该异常通常是协议错误导致:比如构造HttpGet对象时传入协议不对(将'http'写成'htp')or响应内容不符合HTTP协议要求等
            logger.error("请求通信[" + url + "]时协议异常,堆栈轨迹如下", cpe);
        } catch (ParseException pe) {
            logger.error("请求通信[" + url + "]时解析异常,堆栈轨迹如下", pe);
        } catch (IOException ioe) {
            // 该异常通常是网络原因引起的,如HTTP服务器未启动等
            logger.error("请求通信[" + url + "]时网络异常,堆栈轨迹如下", ioe);
        } catch (Exception e) {
